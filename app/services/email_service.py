...
def _draw_flight_card(pdf, title, data):
...
def generate_ticket_pdf_for_passenger(
    ...
    pdf.set_text_color(100, 50, 200)
    pdf.cell(0, 10, f"  Booking ref: {booking_details['number']}  ", ln=True, align='R', fill=True)
    pdf.ln(5)
...
def send_booking_confirmation_email(
    ...
    msg = EmailMessage()
    msg['Subject'] = f'AirShero Ticket - {booking_details["number"]}'
    ...
    for passenger in passengers:
        pdf_path = generate_ticket_pdf_for_passenger(
            booking_id, passenger, booking_details, flight_info
        )
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as smtp:
            ...
            print(f"[email] {count} ticket(s) sent to {to_email}")
